...
class split_merge
{
    ...
    --------------------------------------------------------------------*/
    function split_file($fid,$fpath,$file_name,$parts_num)
    {
        $handle = fopen("$fpath/$file_name", 'rb') or die("error opening file");
        $file_size = filesize("$fpath/$file_name");
        ...
        {
		$fn='F'.$fid.'_s'.$i;
            $handle = fopen("$fpath/$fn", 'wb') or die("error opening file for writing");
